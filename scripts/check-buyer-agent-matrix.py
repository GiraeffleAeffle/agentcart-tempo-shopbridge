        ...
        for evidence_id in evidence:
            ...
            errors.extend(
                ...
                for error in evidence_file_errors(
                    evidence_file,
                    expected_scope="buyer_agent_runtime",
                    expected_owner_id=runtime_id,
                )
            )
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=pathlib.Path,
        help=(
            "Optional evidence directory. With --require-evidence, each required evidence item must exist as "
            "<runtime>/<evidence>.md and pass placeholder, metadata, and substantive-content validation."
        ),
    )
    parser.add_argument(
        "--require-evidence",
