...
Build the decision record from the report instead of copying gate status by
hand. For a no-go decision with blocking gaps:
...
For a go decision, the report must have `status: "passed"` and the operator
must record the beta scope, rollback owner, support channel, and observation
window:
...
python3 scripts/build-beta-release-decision.py \
  ...
  --decision go \
  --operator "<name>" \
  --beta-scope "one supervised staging merchant, USD-only, named buyer-agent runtimes" \
  --rollback-owner "<name or team>" \
  --support-channel "<monitored channel>" \
  --observation-window "14 days" \
  --accepted-risk "<risk accepted by the operator>" \
  --out pilot-release-decision.md
```

The command refuses `go` when the evidence report failed, and it refuses a
blocked no-go decision unless follow-up issue URLs or blocker owners are listed.
...
- `pilot-release-decision.md` generated by
