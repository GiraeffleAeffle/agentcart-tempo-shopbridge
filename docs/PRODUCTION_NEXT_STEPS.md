...
| Standards alignment | AgentCart has a stable commerce core, MPP-shaped checkout, merchant registry/domain proof, rail-neutral verifier contract, skill-only buyer path, audit import/export, registry transparency state, configured-only manifest protocol profiles, and an x402 exact-payment compatibility shim | Standards-ready retail profile with adapters for x402/MPP payments, ERC-8004 identity, ERC-8128 signed HTTP requests, AP2/ACP/UCP/MCP/A2A clients, and ERC-8183 custom jobs | Add signed HTTP request verification |
...
3. **Manifest profiles alpha**: manifests declare configured AgentCart,
   x402/MPP, Stripe/card MPP, and registry profiles, while signed-request
   profiles stay hidden until implemented.
