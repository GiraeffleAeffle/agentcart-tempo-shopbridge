...
| --- | --- | --- | --- |
| WooCommerce plugin hardening | Demo-capable ShopBridge plugin with catalog, quote, order, status, refund metadata, verifier hook, admin readiness checks, and merchant-controlled product exposure modes | Installable merchant plugin with strict auth, idempotency, replay protection, richer product controls, privacy defaults, tests | Add idempotency/replay tests, per-product quantity limits, and restricted-goods controls |
