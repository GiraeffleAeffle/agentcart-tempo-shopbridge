...
| Standards alignment | AgentCart has a stable commerce core, MPP-shaped checkout, merchant registry/domain proof, rail-neutral verifier contract, skill-only buyer path, audit import/export, registry transparency state, and configured-only manifest protocol profiles | Standards-ready retail profile with adapters for x402/MPP payments, ERC-8004 identity, ERC-8128 signed HTTP requests, AP2/ACP/UCP/MCP/A2A clients, and ERC-8183 custom jobs | Add x402 compatibility shim |
