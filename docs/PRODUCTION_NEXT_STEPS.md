...
| --- | --- | --- | --- |
| WooCommerce plugin hardening | Demo-capable ShopBridge plugin with catalog, quote, order, status, refund metadata, verifier hook | Installable merchant plugin with strict auth, per-product opt-in, idempotency, admin readiness checks, privacy defaults, tests | Add merchant readiness checklist, per-product eligibility metadata, and idempotency/replay tests |
