...
| Merchant discovery registry | The shared trust/projection modules, immutable archive, recurring finalized indexer, direct contract-querying buyer skill, bounded buyer-query-seeded candidate sampler, and opt-in fail-closed cross-RPC comparison/alert mechanism are implemented. Direct discovery separates complete lifecycle replay from record-scoped eligibility failure, pins the deployment boundary, and applies chain-specific finality age. The Tempo Moderato contract completed a finalized register/revoke/recover lifecycle; dRPC and Tenderly independently reproduced the four-event history. A Myotis profile is implemented but remains blocked on its Rust adapter exposing the parsed finalized execution height. | Public identity/integrity registry with no private demand or catalog data on-chain, replaceable indexers, independently reproducible finalized state, optional buyer self-verification without a full node, spam-resistant candidate membership, shared UTS-46 handling for IDN merchants, and production governance. | Define the fixed non-ranking anti-spam registration policy; add one shared UTS-46 implementation before admitting IDN domains; activate the independent witness/alert receiver; resolve and drill the Myotis finality adapter; then obtain independent contract/governance review and test with a non-maintainer buyer before selecting a production network. |
...
trust checks for eligibility. The EUR entry remains
curated/offchain; the USD entry is controller-bound to the live testnet contract
and currently active on recovered record hash
