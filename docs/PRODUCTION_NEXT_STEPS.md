...
| Merchant discovery registry | The shared trust/projection modules, immutable archive, recurring finalized indexer, Direct Skill auto-discovery, and opt-in fail-closed cross-RPC comparison/alert mechanism are implemented. The Tempo Moderato contract completed a finalized register/revoke/recover lifecycle; the skill enforced each state, and dRPC plus Tenderly independently reproduced the hosted four-event history. | Public identity/integrity registry with no private demand or catalog data on-chain, replaceable indexers, independently reproducible finalized state, and production governance. | Activate the packaged witness with an independent full-history provider and alert receiver, publish contract source after explicit approval, obtain independent security/governance review, and run the package with a non-maintainer buyer before selecting a production network. |
...
reads the Tempo Moderato RPC `finalized` boundary, is pinned to the published
runtime digest, and is consumed automatically by the Direct Skill. The EUR
entry remains curated/offchain; the USD entry is controller-bound to the live
testnet contract and currently active on recovered record hash
