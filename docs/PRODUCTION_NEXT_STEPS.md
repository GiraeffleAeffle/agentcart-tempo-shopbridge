...
| --- | --- | --- | --- |
| WooCommerce plugin hardening | Demo-capable ShopBridge plugin with catalog, quote, order, status, refund metadata, verifier hook, admin readiness checks, merchant-controlled product exposure modes, product-level max quantity, and checkout exclusion override | Installable merchant plugin with strict auth, idempotency, replay protection, richer product controls, privacy defaults, tests | Add idempotency/replay tests, category rules, stock holds, and structured restricted-goods metadata |
