...
| Merchant discovery registry | The shared trust/projection modules, immutable archive, direct contract-querying buyer skill, bounded sampler, onchain category commitments, and fail-closed cross-RPC comparison are implemented. Tenderly is live as the recurring independent witness, the public projection is labeled `independently_verified` only after agreement, an authenticated receiver passed firing/resolved delivery, and both contracts have public exact-match source. The Daybreak Blue review found no critical issue and its high trust finding is fixed. | Public identity/integrity registry with no private demand or live catalog data on-chain, replaceable routing indexes, independently reproducible finalized state, optional buyer self-verification without a full node, spam-resistant candidate membership, shared UTS-46 handling for IDN merchants, and production governance. | Implement production-v2 bounded validator/admission and candidate-backfill behavior; complete the pinned Myotis drill; configure secondary paging; obtain external human review and non-maintainer evidence; then accept ADR 0012 with named governance before either production deployment. |
...
1. implement and review the production-v2 registry changes and name the Safe
   operators required by ADR 0012;
2. configure the payment-verifier alert webhook and a secondary registry paging
   destination;
3. complete the pinned Myotis sync/restart/freshness drill;
...
6. use the pilot evidence to accept or revise proposed ADR 0012 before any
   production-network write.
