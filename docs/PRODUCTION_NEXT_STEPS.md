...
| Stripe/card, Tempo/USD, and EUR settlement | The external verifier contract, positive/negative payment and refund fixtures, public-URL guard, structured logs/metrics/alerts, and fail-closed SQLite replay store are implemented. The pinned non-root verifier image and Helm workload support a Tempo-only USD profile without requiring Stripe configuration, persistent replay state, secret references, and restricted network policy. The image has been built as a checked OCI archive, but is not yet published or deployed to the Talos pilot because the repository has no currently authenticated writable image registry. | External verifier validates real Stripe/card credentials or Tempo/x402 token transfers, executes settlement/refunds, binds results to the final quote, and survives restart without losing replay protection. | Authenticate a writable registry, publish the verifier by digest, deploy the USD profile, then record payment, refund, and persistent-volume restart evidence before adding EUR rails. |
| Merchant discovery registry | One shared trust module now enforces stable claims, controller-bound domain proof, endpoint scope, payment binding, freshness, revocation, and onchain identity across the gateway, Direct Skill, and registry helper. A shared projection replays complete contract lifecycle events from a finalized envelope, retains immutable historical record hashes for revocation/recovery, and fails closed. The RPC indexer records finalized block identity; chart 0.3.0 can refresh and atomically serve only complete snapshots from a least-privilege sidecar; the public chart can expose immutable record documents; the Direct Skill discovers and verifies the same-origin onchain feed automatically. The v1 contract is deployed empty on Tempo Moderato as a trusted-operator testnet pilot. | Public identity/integrity registry with no private demand or catalog data on-chain, replaceable indexers, independently reproducible finalized state, and production governance. | Publish the pinned runtime and activate the recurring feed; after the hardened USD deployment, register the prepared merchant, wait for finalized state, prove buyer discovery, revoke it, recover with a new hash, and save the complete lifecycle evidence. |
...
maintainer-curated discovery slice. The local technical package now also
contains the production-shaped trust/indexer boundary and its recurring
least-privilege chart runtime, but that new onchain feed has not been enabled
on the public service yet because its runtime image is not published. Agents
must therefore treat the currently hosted feed as curated alpha data, not
...
1. publish the already-built verifier runtime to an authenticated registry,
   deploy the pinned digest to the USD pilot namespace, and activate the
   recurring finalized registry feed with the same digest;
2. record a real quote-bound Tempo payment, refund, and replay-store restart;
3. register the pilot record, index only finalized logs, verify skill-only
