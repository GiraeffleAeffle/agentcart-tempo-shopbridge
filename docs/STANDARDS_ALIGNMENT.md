...
- controller-bound `onchain_identity` metadata that binds the record to its
  chain, registry contract, record id, controller, and committed hash; hosted
  compatibility records may still omit it, but default onchain discovery may
  not.
...
- the same full record shape can still be fetched from a merchant bundle or
  hosted compatibility feed, while contract events remain the default
  authority for candidate membership and lifecycle commitments;
- absent onchain identity metadata is allowed only for explicit hosted/offline
  compatibility input, while malformed or missing identity rejects a record
...
- AP2-style checkout and payment mandate mapping from AgentCart approval
  records;
- UCP-style checkout and A2A-style handoff profile mappings around AgentCart
  quote, approval, checkout, aftercare, and audit concepts;
- translator interfaces for ACP-style cart, authorization, and order concepts;
...
- `scripts/check-ap2-mandate-mapping.py` validates the gate and runtime test
  references.
- `docs/UCP_A2A_PROFILES.md` documents the UCP/A2A mapping boundary;
- `gateway/config/ucp_a2a_profiles.json` keeps the compliance claim explicit as
  `mapping_profiles_not_native_ucp_or_a2a`;
- `scripts/check-ucp-a2a-profiles.py` validates the gate and runtime test
  references.
...
7. UCP/A2A profile mappings. Alpha implemented.
8. Native UCP/A2A runtime adapters. Next.
9. ERC-8183-style escrow/custom-order flow.
