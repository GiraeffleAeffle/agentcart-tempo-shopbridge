...
class ShopBridgePluginContractTests(unittest.TestCase):
    ...
    def test_checkout_mode_can_require_external_verifier_before_token_fallback(self) -> None:
        ...
        self.assertIn("public payment verifier URL", readiness_body)
        self.assertIn("'private_payment_verifier_urls_allowed'", readiness_body)
        self.assertIn("!self::payment_verifier_url_allows_private_networks()", guide_body)
    ...
    def test_registry_connection_can_submit_and_revoke_hosted_records(self) -> None:
        ...
        self.assertIn("wp_safe_remote_post($registry_url", call_body)
        self.assertIn("sanitize_registry_connection_url_setting", call_body)
        self.assertIn("limit_response_size", call_body)
    ...
    def test_registry_health_check_surfaces_remote_registry_state(self) -> None:
        ...
        self.assertIn("wp_safe_remote_get", fetch_body)
        self.assertIn("sanitize_registry_connection_url_setting", fetch_body)
    ...
    def test_verifier_http_calls_reject_redirects_and_redact_errors(self) -> None:
        ...
        self.assertIn("payment_verifier_url_allows_private_networks", SOURCE)
        self.assertIn("payment_verifier_host_resolves_to_public_ips", SOURCE)
        self.assertIn("payment_verifier_ip_is_public", SOURCE)
        self.assertIn("AGENTCART_ALLOW_PRIVATE_PAYMENT_VERIFIER_URL", SOURCE)
        self.assertIn("'reject_unsafe_urls' => !self::payment_verifier_url_allows_private_networks()", helper_body)
        self.assertIn("'redirection' => 0", helper_body)
        self.assertIn("'limit_response_size' => 1048576", helper_body)
        self.assertIn("Payment verifier URL must be a public HTTP(S) URL without embedded credentials.", helper_body)
