...
if [ "${AGENTCART_REQUIRE_DEPLOYMENT_SECRETS:-0}" = "1" ]; then
  for required_name in \
    WOO_PUBLIC_URL \
    AGENTCART_WOO_SHOP_TITLE \
    WOO_ADMIN_USER \
    WOO_ADMIN_PASSWORD \
    WOO_ADMIN_EMAIL \
    AGENTCART_MERCHANT_ID \
    AGENTCART_SHOPBRIDGE_TOKEN; do
    if [ -z "${!required_name:-}" ]; then
      printf 'Required deployment value is missing: %s\n' "$required_name" >&2
      exit 1
    fi
  done
  export AGENTCART_SUPPRESS_DEMO_CREDENTIAL_ECHO=1
fi
